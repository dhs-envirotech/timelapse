...
- `hostapd` & `dnsmasq`: These packages are installable through `apt` and are system deamons to manage the access point. First, we set a static IP in `dhcpcd.conf` which controls the client `dns`. This is based off of [tutorial]
