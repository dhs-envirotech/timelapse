...
def folder_page(folder):
    ...
    files = filter(
        ...
        sorted(os.listdir(app.root_path + '/media/' + folder))
    )
    return render_template('directory.html', folder=folder, folder_uppercase=folders[folders_lowercase.index(folder)], files=files)
